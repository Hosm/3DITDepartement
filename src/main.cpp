...
int main(int argc, char **argv)
{
	...
	for(int i=0; i<objData->faceCount; i++)
	{
		...
		obj_material *m = objData->materialList[o->material_index];
		printf("********************************\nfacette %i : %s with vertex_count:%i \n ",i,m->name,o->vertex_count);
		...
		for(int j=0; j<3; j++)
		{
			...
			printf("\n");
            /*printf("Normal : ------------------");
			printVector(objData->normalList[ o->vertex_index[j] ]);
			printf("\n");*/
            printf("Texture : ------------------");
