...
void obj_set_material_defaults(obj_material *mtl)
{
	...
	mtl->shiny = 0;
	mtl->refract_index = 1;
	//mtl->texture_filenameAmb[0] = '\0';
	//mtl->texture_filenameDif[0] = '\0';
}
...
int obj_parse_mtl_file(char *filename, list *material_list)
{
	...
	while( fgets(current_line, OBJ_LINE_SIZE, mtl_file_stream) )
	{
		...
		{
			current_mtl->emis[0] = atof( strtok(NULL, " \t"));
			current_mtl->emis[1] = atof( strtok(NULL, " \t"));
			current_mtl->emis[2] = atof( strtok(NULL, " \t"));
		}
		...
		{
			current_mtl->filter[0] = atof( strtok(NULL, " \t"));
			current_mtl->filter[1] = atof( strtok(NULL, " \t"));
			current_mtl->filter[2] = atof( strtok(NULL, " \t"));
		}
		//shiny
		...
		{
			current_mtl->invtrans = atof( strtok(NULL, " \t"));
		}
		...
		{
		    char * chemin=strtok(NULL, " \t");
		    size_t i = strlen(chemin)-1;
			strncpy(current_mtl->texture_filenameAmb, chemin, i);
		}
		// texture map ambient
		else if( strequal(current_token, "map_Kd") && material_open)
		{
            char * chemin=strtok(NULL, " \t");
		    size_t i = strlen(chemin)-1;
			strncpy(current_mtl->texture_filenameDif, chemin, i );
		}
		else
		{
...
/*
	...
	while( fgets(current_line, OBJ_LINE_SIZE, obj_file_stream) )
	{
		...
		else if( strequal(current_token, "vt") ) //process vertex texture
		{
		    ...
		    obj_vector* vet = obj_parse_vector();
		    list_add_item(&growable_data->vertex_texture_list,  vet, NULL);
		    //printf("  v->e[0]=%.5f ,v->e[1]=%.5f,v->e[2]=%.5f\n",vet->e[0],vet->e[1],vet->e[2]);
		}
